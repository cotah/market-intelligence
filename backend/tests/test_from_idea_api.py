...
async def test_from_idea_503_when_not_configured(monkeypatch):
    ...
    async with _client() as c:
        r = await c.post("/opportunities/from-idea", json=BODY, headers={"X-API-Key": TEST_KEY})
...
async def test_from_idea_422_on_invalid_body():
    async with _client() as c:
        r = await c.post("/opportunities/from-idea", json={"name": ""}, headers={"X-API-Key": TEST_KEY})
...
async def test_from_idea_enqueues_with_valid_key(monkeypatch):
    ...
    monkeypatch.setattr(w.run_for_idea_task, "delay", lambda idea: FakeTask())
    async with _client() as c:
        r = await c.post("/opportunities/from-idea", json=BODY, headers={"X-API-Key": TEST_KEY})
